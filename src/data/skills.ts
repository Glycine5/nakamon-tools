...
export const rawSkills: Record<string, Skill> = {
  ...
  hastle_break_kai: { name: 'ハッスルブレイク回', raw: [15, 42, 1200, 370, 4, 3, 19, 14] },
...
export const getMonsterStepFamily = (name: string): string => {
  ...
  const idxGaniras = monsterList.indexOf('ガニラス');
  ...
  if (idxKillerMachine !== -1 && idxMaou !== -1 && idx >= idxKillerMachine && idx <= idxMaou) {
    ...
    return '13k';
  } else if (idxDashlan !== -1 && idxGaniras !== -1 && idx >= idxDashlan && idx <= idxGaniras) {
    return '10k';
  } else {
    return 'その他';
  }
